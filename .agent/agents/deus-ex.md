...
model: inherit
skills: deus-dispatch, meridian-routing
---
...
Load `@[skills/deus-dispatch]` first (context + checklist + envelope), then `@[skills/meridian-routing]` for the keyword matrix.

## Phase 0

Follow `deus-dispatch` → `references/project-context.md`. Read-only.
...
Use the table in `.agent/agents/deus-ex.md` only as a reminder. The checklist in `deus-dispatch` is the procedure.
