...
1. Read `docs/discovery/product-brief.md` if it exists (PO context).
2. Read `docs/00_scope.md` if it exists.
2. Read `docs/decisions/` (and stub `11_decisions.md`) for scope-related decisions.
3. If `docs/` missing → recommend `/discover` if intent vague, else escalate to `process-manager` + `init-project`.
